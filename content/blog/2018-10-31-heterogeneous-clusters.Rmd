...
cluster_design <- 
  ...
  # D: Data Strategy
  declare_assignment(Z = cluster_ra(clusters = clusters), legacy = FALSE) +
...
write_rds(simulation_df, file = "rfiles/8_difference-in-means-biased.rds")
```


```{r, echo = FALSE, eval = TRUE, echo = FALSE}
cluster_sims <- read_rds("rfiles/8_difference-in-means-biased.rds") 
```


In the plot below we show the distribution of possible estimates from different possible random assignments. The true treatment effect is 1. We see bias (of size `r round(mean(filter(cluster_sims, estimator == "dim")$estimate - 1), 3)`) from the fact that the distribution is clearly not centered at 1. Very large effects (approx. 3) are estimated in those cases where both of the large clusters get assigned to control (and so all treated outcomes are around 3, right mode) whereas the estimated effects (approx. 0) when both are assigned to treatment are not so small in comparison (left mode), producing right skew. The HT estimator, in contrast, does fine on bias in this example and also has a tighter distribution.
...
write_rds(simulation_df, file = "rfiles/8_difference-in-means-biased2.rds")
```


```{r, echo = FALSE, eval = TRUE, echo = FALSE}
cluster_sims2 <- read_rds("rfiles/8_difference-in-means-biased2.rds") 
...
In this case the bias is of size `r round(mean(filter(cluster_sims2, estimator == "dim")$estimate - 1), 3)`, which is a big reduction.
