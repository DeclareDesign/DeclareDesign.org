...
design_1 <- 
  ...
  declare_assignment(Z = cluster_ra(clusters = classrooms), legacy = FALSE) +
...
ggplot() +
  ...
  stat_density(aes(x=std.error^2, colour=estimator), geom="line", position="identity",
                  data = ests, size = 0.75) +
...
ggplot() +
  geom_vline(aes(xintercept=sd(ests$estimate)^2), linetype="dashed") +
  stat_density(aes(x=std.error^2, colour=estimator),
                  geom="line",
                  position="identity",
                  data = ests, 
                  size = 0.75) +
